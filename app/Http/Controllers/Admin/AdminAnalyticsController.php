...
class AdminAnalyticsController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            $data = Cache::remember('admin_analytics_data', 60 * 5, function () use ($store) {
                if ($store instanceof LockProvider) {
                    return Cache::lock('admin_analytics_data_lock', 30)->block(10, fn () => $this->buildAnalyticsPayload());
                }
    ...
    private function buildAnalyticsPayload(): array
    {
        ...
        $problematicQuestionModels = Question::with('stage')
            ->whereIn('id', $problematicQuestions->pluck('question_id')->all())
            ->get()
            ->keyBy('id');

        $problematicQuestions = $problematicQuestions
            ->map(function ($row) use ($problematicQuestionModels) {
                $question = $problematicQuestionModels->get($row->question_id);
                if (! $question) {
                    return null;
                }
