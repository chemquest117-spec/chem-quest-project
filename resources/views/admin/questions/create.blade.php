<x-app-layout>
               ...
               <form action="{{ route('admin.stages.questions.store', $stage) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div>
                         ...
                         <select name="type" id="question_type" required
                              class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-cyan-500 focus:ring-cyan-500">
                              <option value="mcq" {{ old('type', 'mcq') === 'mcq' ? 'selected' : '' }}>Multiple Choice</option>
                              <option value="complete" {{ old('type') === 'complete' ? 'selected' : '' }}>{{ __('quiz.complete_question') }}</option>
                         </select>
                    ...
                    <div>
                         ...
                         <textarea name="question_text" id="question_text" rows="3" required
                              ...
                              placeholder="{{ __('quiz.complete_question_placeholder') }}">{{ old('question_text') }}</textarea>
                    ...
                    <div>
                         ...
                         <textarea name="explanation" rows="4"
                              class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:border-cyan-500 focus:ring-cyan-500">{{ old('explanation') }}</textarea>
                         @error('explanation') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
                    </div>

                    {{-- MCQ Fields --}}
                    <div id="mcq-fields" class="{{ old('type') === 'complete' ? 'hidden' : '' }} space-y-6">
                         @foreach(['a', 'b', 'c', 'd'] as $opt)
                              <div>
                                   <label class="block text-sm font-medium text-slate-300 mb-2">Option
                                        {{ strtoupper($opt) }}</label>
                                   <input type="text" name="option_{{ $opt }}" id="option_{{ $opt }}" value="{{ old('option_' . $opt) }}"
                                        class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-cyan-500 focus:ring-cyan-500">
                    ...
                    <div id="complete-fields" class="{{ old('type') !== 'complete' ? 'hidden' : '' }} space-y-4">
     ...
     <script>
          document.addEventListener('DOMContentLoaded', function() {
               ...
               const existingAnswers = @json(old('expected_answers', []));
